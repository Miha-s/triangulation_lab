...
void ControllLayer::clear()
{
    lock_guard guard(m_mutex);
    m_control_elements.clear();
}

void ControllLayer::draw(sf::RenderTarget &render_target)
{
...
void ControllLayer::add_element(ControlElementPtr element)
{
    lock_guard guard(m_mutex);
    m_control_elements.push_back(element);
}

bool ControllLayer::consume_event(sf::Event event, sf::Window &window)
{
    for(auto& element : m_control_elements) {
